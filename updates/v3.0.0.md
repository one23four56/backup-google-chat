...
- **Details:**
  ...
  - Bots are now shown in the members tab of a room, rather than their own page
...
- (chromium-based browsers only) The scroll bars on the sidebar show up even when there is nothing to scroll
...
- When old messages are loaded in, you are autoscrolled to the top instead of staying scrolled to where you were
...
**Note:** Rooms were not added until this version, so all bugs listed as affecting rooms actually affected the main chat room, which was the only room
